...
export class Game {
    ...
    public handleShipLoop(shipId: string, getActiveKeys: () => string[], isAutomated: boolean) {
        ...
        if (activeKeys.includes("w")) {
            ...
            const forward = Quaternion.fromAxisAngle([0, 0, 1], -positionTheta + Math.PI / 2).mul(cameraOrientation.clone()).rotateVector([0, 1, 0]);
        ...
        if (!activeKeys.includes(" ") && cameraCannonLoading && faction && cannonCoolDown <= 0) {
            ...
            for (let i = 0; i < shipData.cannons.numCannons; i++) {
                ...
                const positionPoint = cameraPosition.clone().rotateVector([0, 0, 1]);
                const positionTheta = Math.atan2(positionPoint[1], positionPoint[0]);
                const fireDirection = Quaternion.fromAxisAngle([0, 0, 1], -positionTheta + Math.PI / 2).mul(cameraOrientation.clone()).rotateVector(jitterPoint);
